...
class Agent:
    ...
    def receive_observations(self,obs,ts):
        GP.LOG(log_prefix+'[line-13][Check: id(sc)=%d, len(A)=%d]',(self.sc.id, len(self.A)),'optional')
        if self.sc.id is -1 and len(self.A) is 0:
            action = ACT(-1, None)
            self.A.append(action)
            self.A_avai.append(action)
            GP.LOG(log_prefix+'[line-15][initialize an action a[%d], len(A)=%d, len(A_avai)=%d]', (action.id,len(self.A),len(self.A_avai)), 'optional')
        act_value = self.ddpg.act(self.forward_model.predict(self.sc, self.A_avai))
        action = ACT(ts, act_value)
        self.A.append(action)
        GP.LOG(log_prefix+'[agent adds a[%d] into A: len(A)=%d, len(A_avai)=%d]', (action.id, len(self.A), len(self.A_avai)), 'optional')
        return action