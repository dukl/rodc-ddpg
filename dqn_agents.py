...
def concatenate_state_action(state, action):
    #log.logger.debug('state:\n%s' % str(state))
    out = np.concatenate((state, [action]))
    #log.logger.debug('Out: \n %s' % str(out))
    out = np.reshape(out, [1, len(out)])
    #log.logger.debug('concatenate_state_action out: %s' % (str(out)))
    return out
...
class DQNAgent:
    ...
    def memorize(self, state, action, reward, next_state, done):
        if self.is_delayed_agent:
            ...
            #log.logger.debug('Memorize, non-delayed agent')
...
class DDQNAgent(DQNAgent):
    ...
    def train_model(self, batch):
        ...
        #state_vec = np.reshape(state_vec, [3, 25])
        #next_state_vec = np.reshape(next_state_vec, [3, 25])
        target = self.model.predict(state_vec)
        #log.logger.debug('target: %s' % (str(target)))
        ...
        #log.logger.debug('new target: %s' % (str(new_targets)))
        #log.logger.debug('len(batch[0]): %d' % (len(batch[0])))
    ...
    def _create_batch(self, indices):
        ...
        return np.array(state_vec), action_vec, reward_vec, np.array(next_state_vec), done_vec
...
class DDQNPlanningAgent(DDQNAgent):
    ...
    def train_model(self, batch):
        ...
        if self.use_learned_forward_model and self.delay_value > 0:
            #log.logger.debug('Training flowmodel')
    ...
    def act(self, state, pending_actions, eval):
        if not eval and np.random.rand() <= self.epsilon:
            ...
            return random.randrange(self.action_size)
        #log.logger.debug('[DQN][Chose Action With DQN Model]')
        #log.logger.debug('pending actions ... %s' % (str(pending_actions)))
        #log.logger.debug('current state %s' % (str(state)))
        last_state = state
    ...
    def memorize(self, state, action, reward, next_state, done):
        ...
        if len(self.sample_buffer) -1 >= self.delay_value:
            ...
            modified_tuple[3] = next_state
            modified_tuple = tuple(modified_tuple)
            #log.logger.debug('Modified Tuple: \n%s' % str(modified_tuple))
            self.memory.append(modified_tuple)
