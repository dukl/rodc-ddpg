    ...
    def initial_topology(self):
        ...
        for i in range(len(GP.n_NF_inst)):
            for j in range(GP.n_NF_inst[i]):
                ...
                self.n_nf_inst += 1
        GP.LOG(GP.getLogInfo(log_prefix, sys._getframe().f_lineno)+'[Initialize Topology]', None, 'observation')
        for vm in self.vms:
            ...
            GP.LOG(log_tmp, None, 'observation')
...
class NF:
    def __init__(self, type, loc_id, nf_id, inst_id):
        ...
        self.C         = random.random()*30000 + 10000  # current CPU cycles
        self.C_        = self.C
        self.nf_id     = nf_id
